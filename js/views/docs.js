...
const SECTIONS = [
  ...
  { id:'datasource', title:'Data source & backends', body:`
    ...
    <p>Once connected, it’s a <b>write-through mirror</b>: you keep working against a fast local copy, and every change is mirrored up on a short delay (the rail chip shows <b>connected / syncing / error</b>). Open Manager in another browser, connect to the same database, and you pick up right where you left off. <b>Disconnect</b> any time to go back to local-only — your current data stays put. (User accounts and per-user access come later; today the connection is per-browser.)</p>` },
