...
const SECTIONS = [
  ...
  { id:'library', title:'Projects & the library', body:`
    <p>The <b>Projects</b> view is the source of truth. Use the search box, the status dropdown, and the sort controls — or tap a <b>saved view</b> chip (All, Live only, Recently active, Pinned) for a one-click preset. Your last view is remembered.</p>
  ...
  { id:'releases', title:'Releases & what’s new', body:`
    ...
    <p>The <b>Releases</b> section (in the rail) is the fleet-wide view: every project’s releases in one timeline — newest first — with summary stats (last 7/30 days, who shipped) and filters by project, kind, time range, and text. Click any release to jump to that project. Toggle <b>By day / By project</b> to switch between a day-grouped timeline and clusters per project (most recently active first). The rail badge shows how many releases shipped since you last opened this feed, and each one is tagged <b>new</b> until you do — a fleet-wide "since you last looked" marker, separate from the per-project sync preview.</p>
