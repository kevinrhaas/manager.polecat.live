...
const SECTIONS = [
  ...
  { id:'releases', title:'Releases & what’s new', body:`
    ...
    <p><b>Milestones &amp; suggested stable points.</b> Not every release is worth remembering — some are a natural “this is stable, it’s done for now” point. When one release clearly stands out — a run of shipped features, then a stabilizing tail of polish/fix releases, a quiet pause, a round version — Manager marks it right on the <b>What’s new</b> timeline with a quiet <b>Suggested</b> badge (with a tooltip explaining why and its confidence). It’s deliberately rare — at most one per project, often none — a signal, not a nag. Click the badge to accept it as a milestone, the <b>×</b> to dismiss it (dismissing or accepting ends it — it won’t drop to a weaker pick), or flag any release yourself with the 🚩 button (optionally with a label like “1.0” or “Public launch”). Marked milestones get a badge everywhere the release shows, and the fleet-wide <b>Releases</b> feed has a <b>Milestones</b> filter to see just those stable points across every project.</p>
    ...
    <p><b>Sync</b> pulls a project’s real changelog and previews what’s new or changed before importing — safe and additive. <b>Force sync</b> (next to it) is a full reconcile: it overwrites any local edits to a matching release and removes synced releases no longer published upstream, behind a confirm since it’s deliberate. <b>Auto-sync</b> does the safe kind quietly — turn it on fleet-wide in Settings, then opt individual projects in from their health panel, and Manager re-checks them on a cadence without a modal in your way. On top of that, just <b>opening the Projects library</b> kicks a quiet background check of every connected project (throttled, silent), and any project that gains new releases gets a transient <b>NEW</b> badge on its row — it clears when you open that project and fades on its own after about ten days, so you rarely need “Sync all” by hand.</p>
  ...
  { id:'board', title:'4D Board — the reconstruction ticket queue', body:`
    <p>The <b>4D Board</b> is a live window onto the <b>4D Chicago</b> ticket system, which lives in its own repository, <span class="mono">kevinrhaas/chicago-tickets</span> (tickets in folders of 250, <span class="mono">QUEUE.md</span> at the top). Tickets waiting on you are listed first under <b>Needs your decision</b>, each with its question, its options and a recommendation — pick one and it is committed to the ticket for the next run. It reads the generated <span class="mono">tickets.json</span> and lays the tickets out as a board by state — <b>Queue</b>, <b>In progress</b>, <b>Blocked</b>, and <b>Done</b> — with each card showing its epic and whether it was owner-requested, seen, needs a bake, and so on.</p>
    <p>The <b>Queue</b> column is the owner-ordered priority list (top = what the build loop picks up next). Reorder it with the up/down arrows on each card, then <b>Commit order</b> — Manager commits the project’s <span class="mono">QUEUE.md</span> straight to <span class="mono">main</span> of the tickets repo (a compare-and-swap write, so a concurrent edit is refused rather than clobbered). Committing needs the same GitHub token as Fleet Ops; without one the board is read-only. The generated board on GitHub regenerates from your new order on the project’s next ticket run.</p>` },
