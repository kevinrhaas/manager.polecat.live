...
const SECTIONS = [
  ...
  { id:'board', title:'4D Board — the reconstruction ticket queue', body:`
    <p>The <b>4D Board</b> is a live window onto the ticket system in <span class="mono">kevinrhaas/custom</span>’s <b>4D Chicago</b> project (<span class="mono">chicago/4d/tickets/</span>). It reads the generated <span class="mono">tickets.json</span> and lays the tickets out as a board by state — <b>Queue</b>, <b>In progress</b>, <b>Blocked</b>, and <b>Done</b> — with each card showing its epic and whether it was owner-requested, seen, needs a bake, and so on.</p>
    <p>The <b>Queue</b> column is the owner-ordered priority list (top = what the build loop picks up next). Reorder it with the up/down arrows on each card, then <b>Commit order</b> — Manager rewrites the project’s <span class="mono">QUEUE.md</span> on the working branch (a compare-and-swap write, so a concurrent edit is refused rather than clobbered). Committing needs the same GitHub token as Fleet Ops; without one the board is read-only. The generated board on GitHub regenerates from your new order on the project’s next ticket run.</p>` },
